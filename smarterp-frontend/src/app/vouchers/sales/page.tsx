...
export default function SalesVoucherPage() {
  ...
  const [salesLedgers, setSalesLedgers] = useState<Ledger[]>([]);
  ...
  useEffect(() => {
    ...
    setVoucherNumber("SAL-" + Math.floor(1000 + Math.random() * 9000));

    // Load Customers
    apiFetch(`/api/ledgers?companyId=${comp.id}&type=CUSTOMER`)
      .then((data) => {
        setCustomers(data);
        if (data.length > 0) setPartyId(String(data[0].id));
      })
      .catch((err) => setError("Failed to load customers: " + err.message));

    // Load Sales Ledgers
    apiFetch(`/api/ledgers?companyId=${comp.id}`)
      .then((data: Ledger[]) => {
        const filtered = data.filter(l => l.name.toLowerCase().includes("sales"));
        setSalesLedgers(filtered.length > 0 ? filtered : data);
        if (filtered.length > 0) {
          setSalesLedgerId(String(filtered[0].id));
        } else if (data.length > 0) {
          setSalesLedgerId(String(data[0].id));
        }
      })
      .catch((err) => setError("Failed to load sales accounts: " + err.message));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!partyId || !salesLedgerId) {
      setError("Please select a Customer Party and Sales Account.");
      return;
    }

    // Compile double entries
    // DEBIT Customer Party for totalAmount
    // CREDIT Sales Account for subtotal
    // CREDIT GST/Tax Account for tax (or merge into Sales for simplicity if no specific taxes ledger)
    const entries = [];
    ...
    // We can also post stock updates on this line or separate entries.
    // Our service updates stock item if stockItemId is matched.
    // Let's attach stock details on the credit entry line or separate lines.
    // Let's create specific entries mapping line item credits!
    lineItems.forEach((item) => {
      ...
      // Sales Credit
      entries.push({
        ledgerId: Number(salesLedgerId),
        amount: lineAmt + lineGst, // include GST on credit line to balance debit
        entryType: "CREDIT",
    ...
    try {
      ...
      setError(err.message || "Failed to submit sales voucher");
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 py-4 font-mono">
      <div className="border-b border-slate-800 pb-3 flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold text-teal-400">Sales Voucher Creation (F8)</h1>
          <p className="text-slate-500 text-[10px] uppercase mt-1">Post customer invoices and update inventory levels</p>
        </div>
        <button
      ...
      {success && (
        <div className="p-3 bg-teal-500/10 border border-teal-500/20 text-teal-400 text-xs rounded font-bold">
          SUCCESS: Sales voucher posted successfully! Stock quantities updated.
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="bg-slate-900 border border-slate-800 rounded-lg p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-slate-400 uppercase">Party A/c Name (Customer)</label>
            <select
              value={partyId}
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-slate-400 uppercase">Sales Account</label>
            <select
              value={salesLedgerId}
              onChange={(e) => setSalesLedgerId(e.target.value)}
              className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-slate-200 focus:border-teal-500 focus:outline-none text-xs"
            >
              <option value="">Select Sales Acc</option>
              {salesLedgers.map((s) => (
        ...
        <div className="space-y-2 border-t border-slate-800 pt-4">
          <label className="text-[10px] font-bold text-slate-400 uppercase">Inventory / Billing Items</label>
                    ...
                    >
                      ...
                      {stockItems.map((st) => (
                        <option key={st.id} value={st.id}>
                          {st.name} ({st.sku || "No SKU"}) - Available: {st.quantity}
                        </option>
                      ))}
                    </select>
                  </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-slate-800 pt-4">
          <div className="space-y-1">
            ...
            <textarea
              ...
              placeholder="Provide accounting details..."
              rows={3}
              className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-slate-200 focus:border-teal-500 focus:outline-none text-xs"
            />
          </div>
